...
def main():
    ...
    x, y = screen.get_size()
    slider = Slider(screen, int(x/6), int(y/4), int(25), int(y/2), vertical=True, handleRadius=int(x/40), min=4, max=10, step=1)
    font = 'helvetica'
    ...
    while True:
        ...
        FirstStage.renderTextCenteredAt("liczba kolorów", pygame.font.SysFont(font, fsize), (0, 0, 0),
                                        x / 10, y / 4, screen,
                                        x / 10)
        FirstStage.renderTextCenteredAt("Str00p Test\nby RYYYba", pygame.font.SysFont(font, fsize * 4), (0, 0, 0),
                                        x / 2, y / 4, screen,
                                        x / 1.5)
