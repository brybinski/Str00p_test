...
def run(screen, font, fsize, colors_num):
    def add_score(stage, correct=False) -> None:
        ...
        if correct:
            ...
            if tm - GlobalVars.DataCollection[-1][1] != 0:
                ...
                GlobalVars.score = (upper_limit + GlobalVars.score)
            GlobalVars.DataCollection.append([1, tm, to_enter[stage][0], to_enter[stage][0][1], stage])
            return
        GlobalVars.false_count += 1
        if stage < len(to_enter):
            GlobalVars.DataCollection.append([0, tm, to_enter[stage][0], to_enter[stage][0][1], stage])
    ...
    wait_lst = [
        [
            'Wybierz nazwę koloru, który został napisany na ekranie.\n W tej sytuacji Z jest poprawne\n \n Naciśnij ENTER '
            'aby rozpocząć grę',
            [
                level3,
                [
                    [['niebieski', (53, 88, 255)], ['niebieski', (53, 88, 255)]],
                    [
                        [['niebieski', (53, 88, 255)], ['niebieski', (53, 88, 255)]],
                        [['żółty', (254, 175, 22)], ['żółty', (144, 174, 28)]],
                        [['czerwony', (246, 34, 46)], ['czerwony', (246, 34, 46)]],
                        [['zielony', (144, 174, 28)], ['zielony', (144, 174, 28)]]]
                ],
                1.2,
                1,
                1
        ...
        ['Wybierz nazwę koloru, który został napisany na ekranie.\n W tej sytuacji V jest poprawne \n Naciśnij ENTER '
         'aby rozpocząć grę',
         [
             level1,
             [
        ...
        [
            'Wybierz nazwę koloru, który został napisany na ekranie.\n W tej sytuacji X jest poprawne\n \n Naciśnij ENTER '
         'aby rozpocząć grę',
            [
                level3,
                [
    ...
    def gui(clock, time_now):
        ...
        text = pygame.font.SysFont(font, 25).render(f'Poprawnych opowiedzi: {GlobalVars.correct_count}', True,
                                                    (0, 0, 0))
    ...
    def level_test(lst, down_middle=1, down_bottom=1, down_bottom_2=1) -> int:
        # print(lst)
        searched = lst[0]
        answers = lst[1]
        x, y = screen.get_size()
        searched_txt = pygame.font.SysFont(font, fsize * 2).render(searched[0][0], True, searched[0][1])
        text_rect = searched_txt.get_rect(center=(x / 2, y / 2.5 * down_middle))
        screen.blit(searched_txt, text_rect)
        ...
        for i in answers:
            opt = i
            opt_txt = pygame.font.SysFont(font, fsize * 2).render(opt[0][0], True, opt[0][1])
            if opt[0][0] == searched[0][0]:
                corr_answ = it
            text_rect = opt_txt.get_rect(center=(x * (it + 1) / 5, y / 1.5 * down_bottom))
            screen.blit(opt_txt, text_rect)
    ...
    def level_test_2(lst, down_middle=1, down_bottom=1, down_bottom_2=1) -> int:
        # print(lst)
        searched = lst[0]
        answers = lst[1]
        x, y = screen.get_size()
    ...
    for level in level_test, level_test_2, level1, level2, level3:
